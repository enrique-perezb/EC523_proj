...
DATA_PATH = "/projectnb/ec523/projects/proj_GS_LQ_EPB/data/SC_Matricies/combTest/tractography"
AGE_FILE = "/projectnb/ec523/projects/proj_GS_LQ_EPB/data/SC_Matricies/combTest/ages.csv"
BATCH_SIZE = 8
HIDDEN_DIM = 32
LEARNING_RATE = 1e-4
EPOCHS = 50
...
ages_array = np.array(list(ages_dict.values()))
age_min, age_max = ages_array.min(), ages_array.max()
...
# Apply normalization
ages_dict = {k: normalize_age(v) for k, v in ages_dict.items()}
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            for data in test_graphs:
                ...
                pred_denorm = denormalize_age(output)
                target_denorm = denormalize_age(y.view(-1))

                val_loss.append(F.mse_loss(pred_denorm, target_denorm).item())
                val_mae.append(F.l1_loss(pred_denorm, target_denorm).item())
